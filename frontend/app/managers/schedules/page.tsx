...
import { getManagerPerformances, getManagerPerformanceDetails } from "@/lib/api-manager"
    ...
    if (selectedPerformanceId) {
      const fetchPerformanceDetails = async () => {
        try {
          ...
          setError(null)
          const data = await getManagerPerformanceDetails(selectedPerformanceId)
          setSelectedPerformance(data)
        } catch (err) {
          console.error("공연 상세 정보 가져오기 오류:", err)
          setError("공연 상세 정보를 불러오는 중 오류가 발생했습니다.")
  ...
  return (
    <div className="container py-8">
      <div className="flex flex-col gap-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">공연 일정</h1>
          <p className="text-muted-foreground mt-1">등록된 공연의 일정을 확인합니다.</p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>공연 선택</CardTitle>
            <CardDescription>일정을 확인할 공연을 선택하세요.</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
