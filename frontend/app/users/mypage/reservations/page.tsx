...
import type { ReservationResponse, ReservationPageResponse } from "../../../../src/types/reservation"
  ...
  const fetchReservations = async (pageNum: number = 0) => {
    try {
      ...
      if (data && data.content) {
        ...
        setTotalPages(Math.max(1, data.totalPages || Math.ceil(data.totalElements / 10)))
        setPage(pageNum)
      }
    } catch (err) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'PAYMENTS_PENDING':
        return <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200">결제 대기</Badge>
      case 'PAYMENTS_CONFIRMED':
        return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">결제 완료</Badge>
      case 'CANCEL_PENDING':
        return <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200">취소 대기</Badge>
      case 'CANCEL_CONFIRMED':
        return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">취소 확정</Badge>
      default:
          ...
          <Card>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[100px] text-center">예약 번호</TableHead>
                    <TableHead className="text-center w-[200px]">공연명</TableHead>
                    <TableHead className="text-center w-[150px]">공연장</TableHead>
                ...
                <TableBody>
                  {reservations.map((reservation) => (
                    <TableRow key={reservation.reservationId}>
                      <TableCell className="font-medium text-center">{reservation.reservationId}</TableCell>
                      <TableCell className="font-medium text-center truncate max-w-[200px]" title={reservation.title}>{reservation.title}</TableCell>
                      <TableCell className="text-center truncate max-w-[150px]" title={reservation.venue}>{reservation.venue}</TableCell>
                      <TableCell className="text-center">{new Date(reservation.createdAt).toLocaleString()}</TableCell>
                      <TableCell className="text-center">{reservation.quantity}매</TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => router.push(`/users/mypage/reservations/${reservation.reservationId}`)}
                          >
                            상세보기
                          </Button>
