 ...
 */
@Service
class RedisReservationBulkCancelService(
     ...
     */
    @Transactional
    fun cancelAllByPerformanceId(performanceId: Long) {
        log.info { "[공연 일괄 취소] 공연 ID: $performanceId" }

        // 취소된 공연의 모든 회차 id 조회
        val scheduleIds = performanceScheduleRepository.findIdsByPerformanceId(performanceId)
        ...
        // 회차별 예약 목록 조회
        val reservations = reservationRepository.findAllByScheduleIds(scheduleIds)
        ...
        for (reservation in reservations) {
            ...
            // 레디스 좌석 정보 제거
            redisSeatService.deleteSeatStock(reservation.scheduleId)
...
//            smsService.performanceCanceled(reservation);
     ...
     */
    @Transactional
    fun cancelAllByScheduleId(scheduleId: Long) {
        log.info { "[회차 일괄 취소] 회차 ID: $scheduleId" }

        // 회차 예약 목록 조회
        val reservations = reservationRepository.findAllByScheduleId(scheduleId)
