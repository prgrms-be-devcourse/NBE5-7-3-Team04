...
interface PerformanceScheduleRepository : JpaRepository<PerformanceSchedule, Long> {
    ...
    @Query("""
    SELECT new me.performancereservation.domain.performance.model.SchedulePerformanceInfo(p.id, p.title, p.venue, p.price,ps.id, ps.startTime, ps.endTime)
    ...
    """)
    fun findAllSchedulePerformanceInfoByScheduleIds(@Param("scheduleIds") scheduleIds: List<Long>): List<SchedulePerformanceInfo>
