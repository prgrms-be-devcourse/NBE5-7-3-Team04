...
import me.performancereservation.domain.performance.mapper.PerformanceScheduleMapper.toEntity
...
class PerformanceScheduleService (
     ...
     */
    @Transactional
    fun createPerformanceSchedule(performanceId: Long, request: PerformanceScheduleRequest, managerId: Long): Long {
        val performance = performanceRepository.findById(performanceId)
            .orElseThrow {
                ErrorCode.PERFORMANCE_NOT_FOUND.domainException(
                    "해당하는 공연을 찾을 수 없습니다. id=$performanceId"
                )
            }
        ...
        val schedule = toEntity(request, performanceId, performance.totalSeats)
        val savedId = performanceScheduleRepository.save(schedule).id
        // 레디스 좌석 초기화 이벤트 호출
        eventPublisher.publishEvent(PerformanceScheduleCreatedEvent(savedId!!, performance.totalSeats))
     ...
     */
    @Transactional
    fun cancelPerformanceSchedule(performanceId: Long, performanceScheduleId: Long, managerId: Long): Long? {
        // 취소할 회차 검색
        val schedule = performanceScheduleRepository.findById(performanceScheduleId)
            .orElseThrow {
                ErrorCode.PERFORMANCE_SCHEDULE_NOT_FOUND.domainException(
                    "해당하는 공연 회차를 찾을 수 없습니다. id=$performanceScheduleId"
                )
            }
        ...
        if (!schedule.hasPermission(performance.id)) {
            throw ErrorCode.PERMISSION_DENIED.domainException("회차에 대한 권한이 없습니다.")
        }

        schedule.cancel()
        // 예약 취소 이벤트 호출 처리
        eventPublisher.publishEvent(ScheduleCanceledEvent(schedule.id!!))
        return schedule.id
    }
}
