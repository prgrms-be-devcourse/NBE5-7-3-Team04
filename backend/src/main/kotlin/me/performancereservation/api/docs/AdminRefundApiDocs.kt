...
interface AdminRefundApiDocs {
    @Operation(summary = "환불 내역 조회", description = "모든 환불 내역을 조회합니다. 상태별 필터링이 가능합니다.")
    @ApiResponses(
        value = [
            ApiResponse(responseCode = "200", description = "조회 성공"), ApiResponse(
            responseCode = "401",
    ...
    fun getAllRefundDetailsByRefundStatus(
        @Parameter(description = "환불 상태 (선택사항)") status: String?,
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(
            responseCode = "400",
            description = "PENDING 상태가 아닌 환불 (REFUND_STATUS_NOT_PENDING)",
            content = [Content(schema = Schema(implementation = ErrorResponse::class))]
        ), ApiResponse(
            responseCode = "401",
            description = "인증되지 않은 사용자 (UNAUTHORIZED)",
            ...
            responseCode = "404",
            description = "환불 내역을 찾을 수 없음 (REFUND_NOT_FOUND)",
            content = [Content(schema = Schema(implementation = ErrorResponse::class))]
        )]
    )
    fun confirmRefund(
        @Parameter(description = "환불 ID", required = true) refundId: Long
    ): ResponseEntity<Void>
}