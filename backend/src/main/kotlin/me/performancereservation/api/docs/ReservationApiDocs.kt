...
interface ReservationApiDocs {
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(
                responseCode = "404",
                description = "공연 일정을 찾을 수 없음 (PERFORMANCE_SCHEDULE_NOT_FOUND)",
                content = [Content(schema = Schema(implementation = ErrorResponse::class))]
            ),
            ApiResponse(
                responseCode = "409",
                description = "좌석 수량 부족 (INSUFFICIENT_SEATS)",
                content = [Content(schema = Schema(implementation = ErrorResponse::class))]
            )
        ]
    )
    fun reserve(
        @Parameter(description = "예매 정보", required = true) request: ReservationRequest,
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(
                responseCode = "404",
                description = "예매를 찾을 수 없음 (RESERVATION_NOT_FOUND)",
                content = [Content(schema = Schema(implementation = ErrorResponse::class))]
            ),
            ApiResponse(
                responseCode = "409",
                description = "이미 취소된 예매 (RESERVATION_ALREADY_CANCELLED)",
                content = [Content(schema = Schema(implementation = ErrorResponse::class))]
            )
        ]
    )
