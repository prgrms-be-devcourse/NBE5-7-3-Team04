...
interface UserApiDocs {
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(
                responseCode = "409",
                description = "이미 공연자 권한 요청 중 (MANAGER_REQUEST_ALREADY_EXISTS)",
                content = [Content(schema = Schema(implementation = ErrorResponse::class))]
            )
        ]
    )
    fun submitManagerRequest(
        @Parameter(description = "인증된 사용자 정보") @AuthenticationPrincipal principal: CustomOAuth2User,
        @Parameter(description = "공연 관리자 요청 정보") @RequestBody request: UserManagerRequestRequest
    ): ResponseEntity<Void>
    ...
    fun onboard(
        @Parameter(description = "인증된 사용자 정보") @AuthenticationPrincipal principal: CustomOAuth2User,
        @Parameter(description = "온보딩 정보", required = true) request: UserOnboardingRequest
    ): ResponseEntity<UserResponse>
}