...
interface SettlementApiDocs {
    ...
    fun createSettlement(
        @Parameter(description = "정산 생성 정보", required = true) request: SettlementRequest
    ): ResponseEntity<Long>
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(
                responseCode = "400",
                description = "잘못된 요청 (INVALID_REQUEST 또는 이미 승인된 정산)",
                content = [Content(schema = Schema(implementation = ErrorResponse::class))]
            ),
            ApiResponse(
                responseCode = "401",
            ...
            ApiResponse(
                responseCode = "404",
                description = "존재하지 않는 정산 (SETTLEMENT_NOT_FOUND)",
                content = [Content(schema = Schema(implementation = ErrorResponse::class))]
            )
        ]
    )
    fun updateSettlementBankInfo(
        @Parameter(description = "정산 수정 정보", required = true) request: SettlementUpdateRequest
    ...
    fun findSettlementIdByPerformanceId(
    ...
    fun getAllSettlementsWithUserId(
