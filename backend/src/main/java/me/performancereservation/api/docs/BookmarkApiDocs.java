...
public interface BookmarkApiDocs{
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "409", description = "이미 북마크된 공연 (DUPLICATE_BOOKMARK)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class))
        )
    })
    ResponseEntity<Void> performanceBookmark(
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "404", description = "북마크를 찾을 수 없음 (BOOKMARK_NOT_FOUND)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class))
        )
    })
    ResponseEntity<Void> performanceBookmarkCancel(
        @Parameter(description = "인증된 사용자 정보") @AuthenticationPrincipal CustomOAuth2User principal,
        @Parameter(description = "공연 ID", required = true) Long performanceId
    );
    ...
    ResponseEntity<Page<BookmarkedPerformancePageResponse>> getBookmarkedPerformances(
        @Parameter(description = "인증된 사용자 정보") @AuthenticationPrincipal CustomOAuth2User principal,
