...
public interface AdminSettlementApiDocs {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "404", description = "정산을 찾을 수 없음 (SETTLEMENT_NOT_FOUND) 또는 공연을 찾을 수 없음 (PERFORMANCE_NOT_FOUND)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class))
        )
    })
    ResponseEntity<SettlementResponse> confirmSettlement(
        @Parameter(description = "정산 ID", required = true) Long settlementId
    );
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "400", description = "유효하지 않은 정산 상태 (INVALID_SETTLEMENT_STATUS)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class))
        )
    })
    @PageableAsQueryParam
    ResponseEntity<Page<SettlementResponse>> getAllSettlementsWithStatus(
        @Parameter(description = "정산 상태 (PENDING 또는 CONFIRMED). null인 경우 전체 조회") String status,
