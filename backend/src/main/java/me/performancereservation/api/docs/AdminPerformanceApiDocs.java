...
public interface AdminPerformanceApiDocs {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "400", description = "PENDING 상태가 아닌 공연 (PERFORMANCE_STATUS_NOT_PENDING)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class))
        ),
        @ApiResponse(responseCode = "401", description = "인증되지 않은 사용자 (UNAUTHORIZED)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class))
    ...
    ResponseEntity<Void> confirmPerformance(
    ...
    ResponseEntity<Void> rejectPerformance(
        @Parameter(description = "공연 ID", required = true) Long performanceId
    );
    ...
    @PageableAsQueryParam
    ResponseEntity<Page<PendingManagerRequestPageResponse>> pendingManagerRequests(
        @ParameterObject Pageable pageable
    );
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "204", description = "승인 성공"),
        @ApiResponse(responseCode = "400", description = "PENDING 상태가 아닌 요청 (MANAGER_REQUEST_STATUS_NOT_PENDING)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class))
        ),
        @ApiResponse(responseCode = "401", description = "인증되지 않은 사용자 (UNAUTHORIZED)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class))
        ...
        @ApiResponse(responseCode = "404", description = "요청을 찾을 수 없음 (MANAGER_REQUEST_NOT_FOUND) 또는 사용자를 찾을 수 없음 (USER_NOT_FOUND)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class))
        )
    })
    ResponseEntity<Void> approveManagerRequest(
        @Parameter(description = "관리자 요청 ID", required = true) Long managerRequestId
    );
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "404", description = "요청을 찾을 수 없음 (MANAGER_REQUEST_NOT_FOUND)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class))
        )
    })
    ResponseEntity<Void> rejectManagerRequest(
        @Parameter(description = "관리자 요청 ID", required = true) Long managerRequestId
    );
} 