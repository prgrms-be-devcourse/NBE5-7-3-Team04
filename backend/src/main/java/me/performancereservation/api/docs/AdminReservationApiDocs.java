...
public interface AdminReservationApiDocs {
    ...
    ResponseEntity<Page<AdminReservationPageResponse>> getReservations(
    ...
    @PageableAsQueryParam
    ResponseEntity<Page<AdminReservationPageResponse>> searchReservations(
        @ParameterObject Pageable pageable,
        @Parameter(description = "회원 이름") String userName,
        @Parameter(description = "공연 제목") String performanceTitle,
        @Parameter(description = "예매 상태") ReservationStatus reservationStatus,
        @Parameter(description = "시작 날짜 (ISO 형식: yyyy-MM-dd'T'HH:mm:ss)") LocalDateTime startDate,
        @Parameter(description = "종료 날짜 (ISO 형식: yyyy-MM-dd'T'HH:mm:ss)") LocalDateTime endDate
    );
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "404", description = "예매를 찾을 수 없음 (RESERVATION_NOT_FOUND) 또는 회차를 찾을 수 없음 (PERFORMANCE_SCHEDULE_NOT_FOUND)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class))
        )
    })
    ResponseEntity<Void> confirmReservation(
    ...
    ResponseEntity<Void> bulkCancelByPerformanceId(
